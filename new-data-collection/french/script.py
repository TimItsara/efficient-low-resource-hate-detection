# prep_french.py  (same style as prep_thai.py)
import argparse, os, math, random
import pandas as pd
from pathlib import Path

def normalize_df(df):
    # find text column (same logic)
    text_cols = [c for c in df.columns if c.lower() in {"text","tweet","content","message"}]
    if not text_cols:
        text_cols = [c for c in df.columns if df[c].dtype==object]
    text_col = text_cols[0]

    # find label column (French file uses 'labels' -> binary 0/1)
    lbl_candidates = []
    if "labels" in df.columns: lbl_candidates.append("labels")
    lbl_candidates += [c for c in df.columns if "hate" in c.lower() or c.lower() in {"label","y"} and c not in lbl_candidates]
    if not lbl_candidates:
        raise ValueError("Could not find a label column (looked for 'labels', 'label', 'y', or columns containing 'hate').")
...
def stratified_split(df, n_dev=500, n_test=2000, seed=42):
    ...
    # safety: ensure both classes in dev/test when possible
    for name, d in [("dev", dev), ("test", test)]:
...
def main():
    ...
    ap.add_argument("--input", required=True, help="Path to fr_hf.csv")
